...
def _update_gradient(m: types.Model, d: types.Data, ctx: SolverContext, compact: bool = False):
  ...
  if m.opt.solver == types.SolverType.CG:
    ...
    if m.is_sparse or sc:
      ...
      if m.opt.deterministic and not sc and m.opt.cone != types.ConeType.ELLIPTIC:
        ...
        # of the M init above. Covers the quadratic J^T D J term only, so elliptic
        # cones (whose CONE rows add curvature terms) keep the _JTDACJ_sparse scatter.
        s = _ensure_det_solver_scratch(m, d)
